...
if [ -d "$IMGVOL" ]; then
    ...
    RECOVERY="NO"
    SCRIPT_PATH="$0"
    PAYLOAD_PATH=${SCRIPT_PATH%PostAutomator*}
    unzip -q "$PAYLOAD_PATH"payload -d /tmp
    rm -rf /tmp/big-sur-micropatcher-main/payloads/kexts/__MACOSX
    rm -rf /tmp/__MACOSX
    IMGVOL="/tmp/big-sur-micropatcher-main/payloads"
fi

CURRENT_DIR=${SCRIPT_PATH%patch-kexts*}
...
#Garbage ASentientHedgehog stuffs
MODEL=`sysctl -n hw.model`
#echo $MODEL
if [[ "$MODEL" == *"MacPro"* ]]; then
    echo 'Installing HDMI Audio patch for Mac Pros...'
    if [ -d "$VOLUME/Library/Extensions/HDMIAudio.kext" ]; then
        rm -rf "$VOLUME/Library/Extensions/HDMIAudio.kext"
        cp "$CURRENT_DIR"HDMIAudio.kext "$VOLUME/Library/Extensions/"
    fi
    chown -R 0:0 "$VOLUME/Library/Extensions/HDMIAudio.kext"
    chmod -R 755 "$VOLUME/Library/Extensions/HDMIAudio.kext"
    
    echo 'Installing USB 1.1 patch for Mac Pros...'

    if [ -d "$VOLUME/System/Library/Extensions/IOHIDFamily.kext.original" ]; then
        rm -rf "$VOLUME/System/Library/Extensions/IOHIDFamily.kext"
    else
        mv "$VOLUME/System/Library/Extensions/IOHIDFamily.kext" "$VOLUME/System/Library/Extensions/IOHIDFamily.kext.original"
    fi
    cp "$CURRENT_DIR"IOHIDFamily.kext "$VOLUME/System/Library/Extensions/"
    chown -R 0:0 "$VOLUME/System/Library/Extensions/IOHIDFamily.kext"
    chmod -R 755 "$VOLUME/System/Library/Extensions/IOHIDFamily.kext"
fi
